...
const PROMPT = `Search the web for CURRENT cars listed for sale in the UK right now that look UNDERPRICED
...
{"title":"year make model, mileage","url":"listing url from search results or null","source":"site name",
...
function parseFinds(text: string): ScoutFind[] {
    ...
    .map((it) => {
      ...
      return {
        dedupe_key: createHash('sha1')
          .update(url || `${title.toLowerCase()}|${asking ?? ''}`)
          .digest('hex'),
        title,
        url,
...
export async function runScoutScan(): Promise<ScoutFind[]> {
  const apiKey = process.env.GEMINI_API_KEY || process.env.NEXT_PUBLIC_GEMINI_API_KEY;
  if (!apiKey) throw new Error('Car Scout is not configured: missing GEMINI_API_KEY.');
